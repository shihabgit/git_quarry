...
class Ion_auth_model extends CI_Model
{
    ...
    // By Shihab   
    function getPersonalDetailsById($id)
    {   $this->db->select('username, password, email, phone');
        $this->db->where('id',$id);
        $result = $this->db->get('auth_users');
    ...
    {   
        $this->db->update('auth_users', $data, $where);
    } 
    
    // By Shihab   
    function editGroup($auth_group,$emp_id)
    {   
        $this->db->update('auth_users_groups', array('group_id'=>$auth_group), array('user_id'=>$emp_id));
    }
    ...
    // By Shihab   
    function is_exists($unique, $id = '')
    {   
        if ($id)
            $this->db->where("id !=", $id);
        $this->db->where($unique);
        
        if($this->db->count_all_results('auth_users'))
            return TRUE;
        return FALSE;
    }
    
    // By Shihab
    function oldPasswordMatch($id, $old)
    {
        return $this->hash_password_db($id, $old);
    }
    
    // By Shihab  
    public function change_password2($id, $old, $new)
    {
       ...
       /* $old_password_matches = $this->hash_password_db($user->id, $old);
        ...
        {*/
            ...
            $successfully_changed_password_in_db = $this->db->update($this->tables['users'], $data, array('id' => $id));
...
//        }
    ...
    function search($groups = NULL)
    {
        $arr = $this->user($groups);
