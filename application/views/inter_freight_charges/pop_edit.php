...
<div id="pop_inter_freight_charges_edit" class="popupBox">
   ...
   <div class="location_dv" style="text-align: center; font-size: 15pt; color: #586D43; font-weight: bolder;"></div>
...
<script type="text/javascript">
   ...
   $(document).ready(function() {
      ...
      //Saving data
      $('#pop_inter_freight_charges_edit .save').click(function() {

         //Showing Loading image till ajax responds
         $("#pop_inter_freight_charges_edit .responseMessage").html('<img src="images/ajax-loader.gif" /> Loading....');
         // Disabling whole page background till Ajax respond.
         $.blockUI(); // Function is defined at "plugins/blockui-master/jquery.blockUI.js"


         var ifc_add_rent = $('#pop_inter_freight_charges_edit #ifc_add_rent').prop('checked') ? 1 : 2;
         var ifc_add_bata = $('#pop_inter_freight_charges_edit #ifc_add_bata').prop('checked') ? 1 : 2;
         var ifc_add_loading = $('#pop_inter_freight_charges_edit #ifc_add_loading').prop('checked') ? 1 : 2;
         var postData = {
            ifc_id: $('#pop_inter_freight_charges_edit #p_key').val(),
            ifc_rent: $('#pop_inter_freight_charges_edit #ifc_rent').val(),
            ifc_bata: $('#pop_inter_freight_charges_edit #ifc_bata').val(),
            ifc_loading: $('#pop_inter_freight_charges_edit #ifc_loading').val(),
            ifc_add_rent: ifc_add_rent,
            ifc_add_bata: ifc_add_bata,
            ifc_add_loading: ifc_add_loading
         };
         $.post(site_url + "inter_freight_charges/edit", postData, function(result) {

            if (result == 1)
            {
               // Hiding save buttun to prevent the chance for re-entering of data.
               $('#pop_inter_freight_charges_edit .save').hide();
               $("#pop_inter_freight_charges_edit .responseMessage").html('<div class="pop_success"><img src="images/success_1.gif" width="15" height="15"  /> Data saved successfully!!!</div>');
               ...
               $("#pop_inter_freight_charges_edit .responseMessage").html(result);
   ...
   function init_pop_inter_freight_charges_edit() {

      // Setting initial values of elements.
      $('#pop_inter_freight_charges_edit .location_dv').html('');
      $('#pop_inter_freight_charges_edit #p_key').val('');
      $('#pop_inter_freight_charges_edit #ifc_rent').val('');
      $('#pop_inter_freight_charges_edit #ifc_bata').val('');
      $('#pop_inter_freight_charges_edit #ifc_loading').val('');
...
// ifc_rent,ifc_add_rent,ifc_bata,ifc_add_bata,ifc_loading,ifc_add_loading

      $('#pop_inter_freight_charges_edit #pop_drag').prop('checked', 'true');
      $('#pop_inter_freight_charges_edit #pop_self_close').prop('checked', 'true');
      $('#pop_inter_freight_charges_edit .save').show();
      $("#pop_inter_freight_charges_edit .responseMessage").html('');
      //Making the popup box draggable.
      $("#pop_inter_freight_charges_edit").draggable();
      dragUndrag($("#pop_inter_freight_charges_edit"));
   }
