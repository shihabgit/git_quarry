...
$(document).ready(function() {
    ...
    $('.tbl_traversor .nextInput').keyup(function(e) {
        ...
        var curRowIndex = $(this).closest('.tbl_traversor').children('tbody').children('tr.tr_traversor').index($(this).closest('tr.tr_traversor'));
		
        var curColIndex = $(this).closest('tr.tr_traversor').children('td').index($(this).closest('td'));
        var lastRow = $(this).closest('.tbl_traversor').children('tbody').children('.tr_traversor:last');
        var lastRowIndex = lastRow.index();
        var lastColIndex = $(this).closest('.tbl_traversor').children('tbody').children('tr.tr_traversor').children('td').children('.nextInput:last').closest('td').index();
        var is_last_col = ((curRowIndex == lastRowIndex) && (curColIndex == lastColIndex)) ? true : false;
        var indexInTable = $(this).closest('.tbl_traversor').children('tbody').children('tr.tr_traversor').children('td').children('.nextInput').index($(this));
		
        var indexInRow = $(this).closest('tr.tr_traversor').children('td').children('.nextInput').index($(this));
		
        var key = e.keyCode || e.which;

        // move back if the key is SHIFT + ENTER combination or Left Arrow Key.
        if ((key == enter && e.shiftKey) || (key == leftArrow)) {
            $(this).closest('.tbl_traversor').children('tbody').children('tr.tr_traversor').children('td').children('.nextInput').eq(indexInTable - 1).focus().select();
        }

        // Move forward if the key is ENTER. In the case of "textarea", allow to insert a new line.
        else if ((key === enter && !$(this).is("textarea")) || (key == rightArrow)) {
        ...
        // Move up if upArrow pressed. In the case of "select" / "textarea", allow traversing though inside the element itself.
            ...
            $(this).closest('.tbl_traversor').children('tbody').children('tr.tr_traversor').eq(curRowIndex - 1).children('td').children('.nextInput').eq(indexInRow).focus().select();
        }
        ...
        // Move up if downArrow pressed. In the case of "select" / "textarea", allow traversing though inside the element itself.
            ...
            if (addRow && is_last_col)
                ...
                $(this).closest('.tbl_traversor').children('tbody').children('tr.tr_traversor').eq(curRowIndex + 1).find('.nextInput').eq(indexInRow).focus().select();
        }
...
// Before calling the keyup event, the keypressed event will be called. So blocking this.
    ...
    $('.tbl_traversor .remove_row').click(function() {
        if ($(this).closest('.tbl_traversor').children('tbody').children('.tr_traversor').length > 1)
        {   
            var myTable = $(this).closest('.tbl_traversor').children('tbody');
    ...
    function createRow(lastRow)
    {
        ...
        if (clearInputs)
        {
            lastRow.children('td').children('.nextInput').val('');
            lastRow.children('td').children('.nextInput:first').focus();
        }
    ...
    {
        row.closest('.tbl_traversor').children('tbody').children('.tr_traversor').each(function(){
